...
export function Footer() {
  return (
    <footer className="bg-surface-container-low border-t border-outline-variant pt-16 pb-8">
      <div className="max-w-[1200px] mx-auto px-6 md:px-10 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10">
        <div>
          ...
          <a
            ...
            className="block mt-4 text-sm text-on-surface-variant hover:text-accent"
          >
            admin@beaconindica.com
          </a>
          <a
            href="mailto:mukulggn1@gmail.com"
            className="block mt-2 text-sm text-on-surface-variant hover:text-accent transition-colors"
          >
            mukulggn1@gmail.com
          </a>
          <a
      ...
      <div className="max-w-[1200px] mx-auto px-6 md:px-10 mt-12 pt-6 border-t border-outline-variant flex flex-col md:flex-row justify-between gap-3 text-sm text-on-surface-variant">
        <span>© 2026 Beacon Indica Pvt Ltd. All rights reserved.</span>
        <span>From Ideas to Institutions. · Recognized by W-AHEAD Trust</span>
      </div>
    </footer>
  );
}